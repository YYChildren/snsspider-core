 ...
 * negatives are not possible (if element is present then test() will never say
 * false). The false positive probability is configurable (default: 5%)
 * depending on which storage requirement may increase or decrease. Lower the
 * false positive probability greater is the space requirement. Bloom filters
 * are sensitive to number of elements that will be inserted in the bloom
 * filter. During the creation of bloom filter expected number of entries must
 * be specified. If the number of insertions exceed the specified initial number
 * of entries then false positive probability will increase accordingly.
 *
...
public class BloomFilter {
	...
	/*
	 * public BloomFilter() { this(Long.SIZE); }
	 */
	...
	public void merge(BloomFilter that) {
		if (this != that && this.numBits == that.numBits
	 ...
	 * implementation does not check for index bounds nor expand the bit set
	 * size if the specified index is greater than the size.
	 */
	public class BitSet {
		private final long[] data;
		private final int ADDRESS_BITS_PER_WORD = 6;

		public BitSet(int bits) {
			this(new long[(bits + Long.SIZE - 1) / Long.SIZE]);
		}
		...
		public void set(int index) {
			data[index >>> ADDRESS_BITS_PER_WORD] |= (1L << index);
		}
		...
		public boolean get(int index) {
			return (data[index >>> ADDRESS_BITS_PER_WORD] & (1L << index)) != 0;
		}
		...
		public void putAll(BitSet array) {
			assert data.length == array.data.length : "BitArrays must be of equal length ("
